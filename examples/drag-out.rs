...
use windows::{
  ...
  Win32::{
    Foundation::{
      BOOL, DRAGDROP_S_CANCEL, DRAGDROP_S_DROP, DV_E_FORMATETC, E_NOTIMPL, E_OUTOFMEMORY, HGLOBAL,
      OLE_E_ADVISENOTSUPPORTED, S_OK,
    },
    System::{
      Com::{
        IAdviseSink, IBindCtx, IDataObject, IDataObject_Impl, IEnumFORMATETC, IEnumSTATDATA,
        DATADIR_GET, DVASPECT_CONTENT, FORMATETC, STGMEDIUM, TYMED, TYMED_HGLOBAL,
      },
      Memory::{
  ...
  fn GetData(&self, pformatetcin: *const FORMATETC) -> ::windows::core::Result<STGMEDIUM> {
    match self.lookup_format(pformatetcin) {
      ...
      Some(idx) => {
        ...
        stgmed.tymed = TYMED(self.fmtetc[idx].tymed as _);
        stgmed.pUnkForRelease = ManuallyDrop::new(None);
        if self.fmtetc[idx].tymed as i32 == TYMED_HGLOBAL.0 {
          stgmed.Anonymous.hGlobal =
            duplicate_global_data(unsafe { self.stgmeds[idx].Anonymous.hGlobal })?
        }
        Ok(stgmed)
      }
    }
    ...
    *control_flow = ControlFlow::Wait;
    ...
    match event {
      ...
      Event::WindowEvent {
        ...
        stgmed.Anonymous.hGlobal = handle;
